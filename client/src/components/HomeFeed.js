...
const HomeFeed = () => {
  ...
  const handleNewTweet = (event) => {
    ...
    if (event.target.value.length > 0 && event.target.value.length < 280) {
      ...
      event.target.value.length > 225
        ? setColor("#ffd633")
        : setColor("#D2D2D7"); //yellow:grey
    } else {
      setButtonState(true);
        ...
        <Wrapper>
          ...
          <SectionBox>
            <StyledForm onSubmit={handleSubmit}>
              ...
              <FormSection2>
                <CharCount style={{ color: countColor }}>
                  {newTweet.numCharLeft}
                </CharCount>
                <StyledButton type="submit" disabled={buttonState}>
                  Meow
                </StyledButton>
              </FormSection2>
            </StyledForm>
