...
const TweetDetails = () => {
  ...
  return (
    <Wrapper>
      ...
      <SectionBox>
        {currentTweet.retweetFrom ? (
          <Retweeted>
            <FiRepeat /> {currentTweet.retweetFrom.displayName} Remeowed
          </Retweeted>
        ) : null}
        <HeaderTweet>
          <Avatar
            src={
              currentTweet.author.handle === "giantcat9"
        ...
        <Content>
          ...
          {currentTweet.media[0] ? (
            <Image src={currentTweet.media[0].url} />
          ) : null}
          <FooterTweet>
            <FooterContent>
              {format(new Date(currentTweet.timestamp), "p")}
            </FooterContent>
            <FooterContent>
              · {format(new Date(currentTweet.timestamp), "MMM d Y")}
            </FooterContent>
